...
public class SearchEditText extends EditText implements OnFocusChangeListener, TextWatcher {
	...
	private void init() {
		...
		if (clearWordsImage == null) {
			clearWordsImage = getResources().getDrawable(R.drawable.edittext_clear_bar);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (getCompoundDrawables()[2] != null) {
			if (event.getAction() == MotionEvent.ACTION_UP) {
				boolean touchable = event.getX() > (getWidth() - getPaddingRight() - clearWordsImage.getIntrinsicWidth()) && (event.getX() < ((getWidth() - getPaddingRight())));
				if (touchable) {
					this.setText("");
				}
			}
